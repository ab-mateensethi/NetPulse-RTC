...
class MonitoringClientGUI(tk.Tk):
    ...
    def build_ui(self):
        ...
        self.private_message_var = tk.StringVar()
        ttk.Label(private_box, text="Target Client Name").pack(anchor="w")
        ttk.Entry(private_box, textvariable=self.private_to_var).pack(fill=tk.X)
        ttk.Label(private_box, text="Private Message").pack(anchor="w", pady=(6, 0))
        ttk.Entry(private_box, textvariable=self.private_message_var).pack(fill=tk.X)
    ...
    def connect(self):
        ...
        if not self.name_var.get().strip():
            messagebox.showwarning("Name required", "Please enter a client name before connecting.")
            return
        try:
            self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.connect_socket_with_auto_server()
            send_json(self.sock, {"type": "hello", "name": self.name_var.get().strip(), "hostname": socket.gethostname()})
            self.connected = True
    ...
    def connect_socket_with_auto_server(self):
    ...
    def start_local_server(self):
        ...
        if os.name == "nt":
            subprocess.Popen(
                [sys.executable, "-B", server_path],
                creationflags=subprocess.CREATE_NEW_CONSOLE,
            )
        else:
            subprocess.Popen([sys.executable, "-B", server_path])
    ...
    def private_message(self):
        target = self.private_to_var.get().strip()
        text = self.private_message_var.get().strip()
        if not target:
            messagebox.showwarning("Target required", "Enter the target client name for private chat.")
            return
        if not text:
            messagebox.showwarning("Message required", "Enter a private message.")
            return
        self.send({"type": "private", "to": target, "text": text})
        self.private_message_var.set("")
